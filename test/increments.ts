...
describe("movement tests", () => {
	pieceMoveTest("[White] will move a2-a3", coord(1,2), coord(1,3), coord(1,3));
	pieceMoveTest("[White] will not move a3-a4 due to being black's turn", coord(1,3), coord(1,4), coord(1,3));
	pieceMoveTest("[Black] will move a7-a6", coord(1,7), coord(1,6), coord(1,6));
	pieceMoveTest("[White] will move a3-a4", coord(1,3), coord(1,4), coord(1,4));
	pieceMoveTest("[Black] will move a6-a5", coord(1,6), coord(1,5), coord(1,5));
	pieceMoveTest("[White] will move not move a4-a5 due to 'cannot capture'", coord(1,5), coord(1,6), coord(1,5));
	pieceMoveTest("[White] will move g1-h3", coord(7,1), coord(8,3), coord(8,3));
	pieceMoveTest("[Black] will move b7-b5", coord(2,7), coord(2,5), coord(2,5));
	pieceMoveTest("[White] will capture from a4-b5", coord(1,4), coord(2,5), coord(2,5));
	pieceMoveTest("[Black] will move c7-c5, enabling enpassant capture on c6", coord(3,7), coord(3,5), coord(3,5));
	pieceAvailableMovesTest("will find all available moves for white pawn on b5", coord(2,5), [coord(2,6), coord(3,6)]);
	pieceMoveTest("[White] will capture EnPassant from b5-c6", coord(2,5), coord(3,6), coord(3,6));
	tagTest("enpassant tag on c6 will be removed after the capture", coord(3,6), "enpassant", undefined);
	pieceMoveTest("[Black] will move Nb8-Na6", coord(2,8), coord(1,6), coord(1,6));
	pieceMoveTest("[White] will move g2 pawn two squares to for enpassant tag", coord(7, 2), coord(7,4), coord(7,4));
	tagTest("will have enpassant tag on g3", coord(7,3), "enpassant", true);
	pieceMoveTest("[Black] will move Bc8-Bb7", coord(3,8), coord(2, 7), coord(2,7));
	pieceMoveTest("[White] will move Bf1-Bg2 to enable white king-side castling", coord(6,1), coord(7,2), coord(7,2));
	pieceMoveTest("[Black] will move Qd8-Qc7 enabling queenside castling", coord(4,8), coord(3,7), coord(3,7));
	pieceAvailableMovesTest("[White] will be able to move Ke1-Kg1 (O-O) and Ke1-Kf1", coord(5,1), [coord(6,1), coord(7,1)]);
	pieceMoveTest("[White] will castle king side (Ke1-Kg1 or O-O)", coord(5,1), coord(7,1), coord(7,1));
	...
	pieceMoveTest("[Black] will castle queen side (Ke8-Kc8 or o-o-o)", coord(5,8), coord(3,8), coord(3,8));
...
function pieceMoveTest(message: string, from: Chess.Coordinate, to: Chess.Coordinate, expected: Chess.Coordinate) {
	var piece: Chess.Piece = classicEngine.getSquare(from).piece;
	
	it (message, () => {
		var moveResult = classicEngine.movePiece(from, to);
		expect(piece.location.file).to.equal(expected.file);
		expect(piece.location.rank).to.equal(expected.rank);
	});
}