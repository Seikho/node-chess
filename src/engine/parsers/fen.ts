...
class FenParser implements Chess.PositionParser {
	...
	parse(position: string): void {
		...
		this.engineInput = fenStringParser.parse(position);

		// Fen strings start from the 8th rank, so we start from 8 and descend to rank 1.
		var rankCount = this.parentEngine.rankCount;
		this.engineInput.ranks.forEach(rank => {
			this.parentEngine.ranks[rankCount] = this.createFilesForRank(rank, rankCount);
			rankCount--;
		});
	}
	...
	getPiece(notation: string): Chess.Piece {
		var pieceFactory = this.parentEngine.pieces.filter(p => p.notation.toUpperCase() === notation || p.notation.toLowerCase() === notation);
