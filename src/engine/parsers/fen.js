...
var FenParser = (function () {
    function FenParser(parentEngine) {
    ...
    FenParser.prototype.parse = function (position) {
        ...
        this.engineInput.ranks.forEach(function (rank) {
            _this.parentEngine.ranks[rankCount] = _this.createFilesForRank(rank, rankCount);
            rankCount--;
        });
    };
    ...
    FenParser.prototype.getPiece = function (notation) {
        var pieceFactory = this.parentEngine.pieces.filter(function (p) { return p.notation.toUpperCase() === notation || p.notation.toLowerCase() === notation; });
